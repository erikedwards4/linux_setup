...
sudo NEEDRESTART_MODE=a apt-get -y upgrade          # uses the updated package list to update packages on the system (takes a long time the first time)
sudo NEEDRESTART_MODE=a apt-get -y dist-upgrade     # same as upgrade, but also handles changing dependencies (probably does nothing here, but just in case)
